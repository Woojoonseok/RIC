...
const {
  ...
  graph, relationPaths, layouts, styles, selectedLayers, selectedRelation, selectedTexts, ports, viewBoxString, portPoint, portHandlePoint, layerLabelLines,
...
<template>
  <div class="canvas-pane">
    ...
    <svg ref="svg" class="canvas" :class="`${app.mode}-mode`" :viewBox="viewBoxString" @pointerdown="canvasDown" @pointermove="pointerMove" @pointerup="pointerUp" @pointercancel="pointerUp" @wheel="wheel">
      ...
      <g v-for="layer in graph?.layers" :key="layer.id" class="layer-node" :class="{ selected: selectedLayers.has(layer.id), pinned: layouts.get(layer.id)?.pinned, 'connect-source': connect?.layerId === layer.id, 'connect-candidate': app.mode === 'connect' && Boolean(connect) && connect?.layerId !== layer.id }" @pointerdown="nodePointerDown($event, layer.id)" @dblclick.stop="editLayer(layer.id)">
        <template v-if="layouts.get(layer.id)"><rect :x="layouts.get(layer.id)!.x" :y="layouts.get(layer.id)!.y" :width="layouts.get(layer.id)!.width" :height="layouts.get(layer.id)!.height" rx="12" :fill="styles.get(layer.id)?.fill_color || '#fff'" :stroke="styles.get(layer.id)?.stroke_color || '#175cd3'" :stroke-width="styles.get(layer.id)?.stroke_width || 2"/><text :x="layouts.get(layer.id)!.x + layouts.get(layer.id)!.width / 2" :y="layouts.get(layer.id)!.y + layouts.get(layer.id)!.height / 2" text-anchor="middle" dominant-baseline="middle" :font-size="styles.get(layer.id)?.font_size || 14"><tspan v-for="(line, index) in layerLabelLines(layer)" :key="index" :x="layouts.get(layer.id)!.x + layouts.get(layer.id)!.width / 2" :dy="index ? 18 : -(layerLabelLines(layer).length - 1) * 9" :fill="line.color">{{ line.text }}</tspan></text>
          ...
          <foreignObject v-if="layouts.get(layer.id)!.pinned" class="layer-pin" :x="layouts.get(layer.id)!.x + layouts.get(layer.id)!.width - 25" :y="layouts.get(layer.id)!.y + 7" width="18" height="18"><Pin :size="16" aria-label="위치 고정"/></foreignObject>
      ...
      <g v-for="text in annotationTexts" :key="text.id" class="text-box" :class="{ selected: selectedTexts.has(text.id) }" @pointerdown="textPointerDown($event, previewTexts[text.id] || text)" @dblclick.stop="editTextBox(text.id)"><rect :x="(previewTexts[text.id] || text).x" :y="(previewTexts[text.id] || text).y" :width="(previewTexts[text.id] || text).width" :height="(previewTexts[text.id] || text).height" :fill="text.background_color" :stroke="text.border_color"/><text :x="(previewTexts[text.id] || text).x + 8" :y="(previewTexts[text.id] || text).y + (previewTexts[text.id] || text).height / 2" dominant-baseline="middle" :fill="text.text_color" :font-size="text.font_size">{{ text.text }}</text><rect v-if="selectedTexts.has(text.id) && !text.locked" class="resize-handle" :x="(previewTexts[text.id] || text).x + (previewTexts[text.id] || text).width - 6" :y="(previewTexts[text.id] || text).y + (previewTexts[text.id] || text).height - 6" width="12" height="12" @pointerdown.stop="textPointerDown($event, previewTexts[text.id] || text, true)"/></g>
