...
import type { LayerImpactNode, LayerImpactReport } from "../../types";

const props = defineProps<{ layerId: string }>();
const app = useAppStore();
const project = useProjectStore();
const impact = ref<LayerImpactReport | null>(null);
const loading = ref(false);
const error = ref("");
let loadNonce = 0;

async function load() {
  if (!project.projectId || !props.layerId) return;
  const nonce = ++loadNonce;
  loading.value = true;
  error.value = "";
  try {
    const result = await api.layerImpact(project.projectId, props.layerId);
...
watch(() => [props.layerId, project.currentRevision], () => void load(), { immediate: true });
</script>
...
<template>
  <section class="layer-impact-summary">
    ...
    <template v-else-if="impact">
      ...
      <div v-if="impact.upstream_layers.length" class="layer-impact-path">
        <span><ArrowUp :size="12"/> Upstream</span>
        <button v-for="row in impact.upstream_layers" :key="row.id" @click="selectLayer(row)">{{ row.name }}</button>
      </div>
      <div v-if="impact.downstream_layers.length" class="layer-impact-path">
        <span><ArrowDown :size="12"/> Downstream</span>
        <button v-for="row in impact.downstream_layers" :key="row.id" @click="selectLayer(row)">{{ row.name }}</button>
      </div>
      <details v-if="impact.validation_rules.length" class="layer-impact-rules">
        <summary>영향받는 검증 규칙 {{ impact.validation_rules.length }}개</summary>
        <p v-for="rule in impact.validation_rules" :key="rule.id"><b :class="rule.severity">{{ rule.severity === 'error' ? '오류' : '경고' }}</b>{{ rule.name }}</p>
      </details>
