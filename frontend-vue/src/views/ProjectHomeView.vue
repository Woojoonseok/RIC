  ...
  <section v-if="current" class="page project-home-page">
    ...
    <template v-if="project.hasMembership">
      <div class="project-home-summary">
        <RouterLink class="panel project-summary-card" :to="{ name: 'align-tree-list', params: { projectId: current.id } }"><span>ALIGN TREES</span><strong>{{ current.align_tree_count ?? project.alignTrees.length }}</strong><small>목록에서 생성하고 편집하기 →</small></RouterLink>
