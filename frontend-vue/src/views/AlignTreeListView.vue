  ...
  <section class="page align-tree-list-page project-tree-page">
    ...
    <div class="tree-card-grid">
      <article v-for="tree in project.alignTrees" :key="tree.id" class="panel tree-card">
        <div class="tree-card-top"><span class="tree-symbol">⌘</span><div class="align-tree-badges"><span v-if="tree.is_default" class="access-badge owner">Default</span><span v-if="project.leaseState === 'locked' || (project.currentProject?.is_locked && !project.currentProject?.locked_by_me)" class="lock-badge">다른 사용자가 편집 중</span><span v-else-if="project.leaseState === 'held'" class="lock-badge mine">내 편집 세션</span></div></div>
