...
Legacy 프로젝트 수동 귀속은 기본적으로 비활성화되어 있다. DB를 다른 서버로 옮긴 뒤에는 서버를 외부에 열기 전에 `ALLOW_LEGACY_PROJECT_CLAIMS=true`로 잠시 실행하고, 새 서버의 브라우저에서 `기존 프로젝트 가져오기`를 눌러 현재 사용자를 Owner로 지정한 다음 즉시 다시 `false`로 되돌린다. 향후 AD SSO를 도입할 때는 기존 내부 Actor에 AD identity를 연결해 프로젝트 소유권과 공유 권한을 유지한다. 현재의 비보안 쿠키 설정은 임시 사내 HTTP 환경 전용이다. **AD SSO 도입 또는 production 배포 전에는 TLS(HTTPS)를 반드시 적용하고 `IDENTITY_COOKIE_SECURE=true`로 설정해야 한다.**
